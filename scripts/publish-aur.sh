...
INSTALL_FILE=$REPO_ROOT/packaging/aur/modulejail.install
...
cp "$INSTALL_FILE" "$AUR_PUBLISH_DIR/modulejail.install"
...
git -C "$AUR_PUBLISH_DIR" add PKGBUILD LICENSE modulejail-signing-key.gpg modulejail.install .SRCINFO
